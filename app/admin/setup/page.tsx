...
export default function AdminSetupPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        setError("You need to be logged in as an admin to add another account.")
      } else {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-white dark:bg-black px-4">
      ...
      >
        <h1 className="text-2xl font-bold text-black dark:text-white text-center">Create Admin Account</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 text-center">
          Pick your own username and password. If this is the first account ever created, you'll be logged in
