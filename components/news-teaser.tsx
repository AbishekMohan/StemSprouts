        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.map((post, index) => (
            <PopIn key={post.id} delay={(index % 3) * 120} className="h-full">
              <Link
                href={`/news/${post.slug}`}
                className="h-full flex flex-col bg-white dark:bg-black border-[3px] border-black dark:border-white rounded-[24px] p-6 hover:shadow-[6px_6px_0px_0px_rgba(34,197,94,1)] transition-shadow"
              >
                <div
